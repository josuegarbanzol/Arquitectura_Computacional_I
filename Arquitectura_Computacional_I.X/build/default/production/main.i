...
main:
    ...
    banksel ANSELA
    clrf ANSELA ; ((PORTA) and 07Fh), 0, ((PORTA) and 07Fh), 1 como digital

    ; --- Deshabilitar analog en PORTB ---
    banksel ANSELB
    clrf ANSELB ; ((PORTB) and 07Fh), 0, ((PORTB) and 07Fh), 1 como digital

    ; --- TRISA: ((PORTA) and 07Fh), 0 y ((PORTA) and 07Fh), 1 como entradas ---
    banksel TRISA
    bsf TRISA, 0 ; ((PORTA) and 07Fh), 0 entrada (SW1 modo)
    bsf TRISA, 1 ; ((PORTA) and 07Fh), 1 entrada (SW2 frecuencia)

    ; --- TRISB: ((PORTB) and 07Fh), 0 y ((PORTB) and 07Fh), 1 como salidas ---
    banksel TRISB
    bcf TRISB, 0 ; ((PORTB) and 07Fh), 0 salida (LED GREEN)
    bcf TRISB, 1 ; ((PORTB) and 07Fh), 1 salida (LED RED)
